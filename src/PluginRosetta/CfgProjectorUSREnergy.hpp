...
namespace Antipatrea
{
     ...
     */    
        ...
        public:
            ...
            {
                if (GetMolecularStructureRosetta() == NULL)
                    std::cout << "MolStructRos is null" << std::endl;

                if (GetCfgManager() == NULL)
                    std::cout << "GetCfgManager is null" << std::endl;

                if (GetCfgManager() && !GetCfgManager()->CheckSetup())
                    std::cout << "GetCfgManager is not happy" << std::endl;
                if (GetMolecularStructureRosetta() && !GetMolecularStructureRosetta()->CheckSetup())
                    std::cout << "GetCfgManager is not happy" << std::endl;
